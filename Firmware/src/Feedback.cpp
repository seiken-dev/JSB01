...
void setPattern(uint16_t first, uint16_t second, uint16_t third,
                uint16_t fourth) {
  p.period_1 = first;
  p.period_2 = second;
  p.period_3 = third;
  p.period_4 = fourth;
  wantClear = true;
  // Serial.printf("%d, %d, %d, %d        \r", p.period_1, p.period_2,
  // p.period_3, p.period_4);
}
...
#endif
{
  ...
  if (expire == 0) {
    ...
    // インターバルの長さをセット
    if (i == 0 && p.period_1)
      expire = millis() + p.period_1;
    else if (i == 1 && p.period_2)
      expire = millis() + p.period_2;
    else if (i == 2 && p.period_3)
      expire = millis() + p.period_3;
    else if (i == 3 && p.period_4)
      expire = millis() + p.period_4;
