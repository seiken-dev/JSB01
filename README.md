...
PlatformIOを使ってビルドすることを想定しています。Firmwareディレクトリでビルドを行ってください。

## 各ブランチはなんのため？

* Release: リリース用ブランチ。これが製品のファームになります。
* c.mos: c.mosさん作成のコード。実装されたハードウェアの機能が一番先に試せるはず
...
* Experimental: 各種実験ブランチ。c.mosブランチの次に動く可能性があるブランチ
